...
// ================= WIFI =================
const char* apSSID = "Truong-Lang-Quan";
...
void handleSend() {
  ...
  String page =
  "<script>alert('[" + a + " | " + b + "]');window.location='/';</script>";
...
void drawUI() {
  ...
  M5.Display.println("idk-portal");
  M5.Display.println("AP:Truong-Lang-Quan");
  M5.Display.println("Next:down Prev:up A:view");
  ...
  if (dataCount == 0) {
    ...
    if (g_idx < g_scroll) g_scroll = g_idx;
    if (g_idx >= g_scroll + visible)
      g_scroll = g_idx - visible + 1;

    size_t end = g_scroll + visible;
    if (end > dataCount) end = dataCount;

    for (size_t i=g_scroll;i<end;i++) {
      M5.Display.setTextColor(
        i==g_idx ? TFT_YELLOW : TFT_WHITE,
        TFT_BLACK
      );
...
void loop() {
  ...
  if (dataCount > 0) {
    ...
    if (M5.BtnB.wasPressed()) {
      g_idx = (g_idx + 1) % dataCount;
      drawUI();
    }

    // PREV
    if (M5.BtnPWR.wasPressed()) {
      g_idx = (g_idx + dataCount - 1) % dataCount;
      drawUI();
    }

    // VIEW
    if (M5.BtnA.wasPressed()) {
      g_status = "View: " + dataList[g_idx];
      drawUI();
    }
  }
