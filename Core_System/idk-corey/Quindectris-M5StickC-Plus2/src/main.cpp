// main.cpp - Quindectris Main Entry Point
...
void showTitleScreen() {
  ...
  if (firstDraw) {
    ...
    // Pick 3 random pieces to display (now from 15 total pieces)
    for (int i = 0; i < 3; i++) {
      animPieces[i] = random(0, 15);
  ...
  // Draw "QUIN15" (Quindectris 15-pieces) in block letters
  int bs = 5; // block size
  ...
  // Q (row 1)
  M5.Lcd.fillRect(x, y+bs, bs*4, bs, COLOR_CYAN);
  M5.Lcd.fillRect(x, y+bs, bs, bs*3, COLOR_CYAN);
  M5.Lcd.fillRect(x+bs*3, y+bs, bs, bs*3, COLOR_CYAN);
  M5.Lcd.fillRect(x, y+bs*4, bs*4, bs, COLOR_CYAN);
  M5.Lcd.fillRect(x+bs*2, y+bs*3, bs*2, bs, COLOR_CYAN);
  
  // U (row 2)
  y += bs*6;
  M5.Lcd.fillRect(x, y, bs, bs*4, COLOR_GREEN);
  M5.Lcd.fillRect(x+bs*3, y, bs, bs*4, COLOR_GREEN);
  M5.Lcd.fillRect(x, y+bs*4, bs*4, bs, COLOR_GREEN);
  
  // I (row 3)
  y += bs*6;
  M5.Lcd.fillRect(x, y, bs*5, bs, COLOR_RED);
  M5.Lcd.fillRect(x+bs*2, y+bs, bs, bs*3, COLOR_RED);
  M5.Lcd.fillRect(x, y+bs*4, bs*5, bs, COLOR_RED);
  
  // N (row 4)
  y += bs*6;
  M5.Lcd.fillRect(x, y, bs, bs*5, COLOR_YELLOW);
  M5.Lcd.fillRect(x+bs*4, y, bs, bs*5, COLOR_YELLOW);
  M5.Lcd.fillRect(x+bs, y+bs, bs, bs, COLOR_YELLOW);
  M5.Lcd.fillRect(x+bs*2, y+bs*2, bs, bs, COLOR_YELLOW);
  M5.Lcd.fillRect(x+bs*3, y+bs*3, bs, bs, COLOR_YELLOW);
  
  // Continue right column for "15"
  x = 75;
  y = 15;
  
  // 1 (row 1)
  M5.Lcd.fillRect(x+bs*2, y, bs, bs*5, COLOR_PURPLE);
  M5.Lcd.fillRect(x+bs, y, bs, bs, COLOR_PURPLE);
  M5.Lcd.fillRect(x, y+bs*4, bs*5, bs, COLOR_PURPLE);
  
  // 5 (row 2)  
  y += bs*6;
  M5.Lcd.fillRect(x, y, bs*5, bs, COLOR_CYAN);
  M5.Lcd.fillRect(x, y+bs, bs, bs, COLOR_CYAN);
  M5.Lcd.fillRect(x, y+bs*2, bs*4, bs, COLOR_CYAN);
  M5.Lcd.fillRect(x+bs*4, y+bs*3, bs, bs, COLOR_CYAN);
  M5.Lcd.fillRect(x, y+bs*4, bs*4, bs, COLOR_CYAN);
  
  // Version badge
  M5.Lcd.fillRect(45, 155, 35, 12, COLOR_WHITE);
  M5.Lcd.setTextSize(1);
  M5.Lcd.setTextColor(COLOR_BLACK);
  M5.Lcd.setCursor(49, 157);
  M5.Lcd.print("v2.0");
