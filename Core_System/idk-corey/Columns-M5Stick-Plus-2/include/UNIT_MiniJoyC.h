...
public:
    bool begin() {
        return _joyc.begin(&Wire, JoyC_ADDR, 0, 26, 100000);
    }
    
    void update() {
        _x = _joyc.getPOSValue(0, _8bit);
        _y = _joyc.getPOSValue(1, _8bit);
        _buttonPressed = _joyc.getButtonStatus();
    }
    
    uint16_t GetX() { return _x; }
    uint16_t GetY() { return _y; }
    bool GetButton() { return _buttonPressed; }
};
