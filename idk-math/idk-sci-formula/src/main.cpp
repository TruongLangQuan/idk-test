...
void actionMath() {
  String out = "Area circle: A=pi r^2\n";
  out += "Quadratic: ax^2+bx+c=0\n";
  out += "Trig: sin^2+cos^2=1\n";
  out += "Derivative: f'(x)\n";
  out += "Integral: int f(x) dx";
  showResult("Math", out);
}
...
void actionChem() {
  ...
  out += "C1V1=C2V2\n";
  out += "pH=-log[H+]\n";
  out += "DeltaG=DeltaH-TDeltaS";
  showResult("Chemistry", out);
}

void actionBio() {
  String out = "DNA->RNA (T->U)\n";
  out += "GC% = (G+C)/total\n";
  out += "Hardy: p^2+2pq+q^2=1";
  showResult("Biology", out);
}

MenuItem kMenu[] = {
    {"Math formulas", actionMath},
    {"Physics formulas", actionPhysics},
    {"Chemistry formulas", actionChem},
    {"Biology formulas", actionBio},
};
...
void drawMenu() {
  ...
  M5.Display.print("idk-sci-formula");
