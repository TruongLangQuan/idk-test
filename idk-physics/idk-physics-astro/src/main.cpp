...
void actionOrbital() {
  ...
  if (!inputNumber(r, "r (m)", g_telex_mode)) return;
  if (r <= 0) { showResult("Orbital", "r>0"); return; }
  double v = sqrt(kG * M / r);
  showResult("Orbital", "v = " + formatFloat(v, 6) + " m/s");
}
...
void actionEscape() {
  ...
  if (!inputNumber(M, "M (kg)", g_telex_mode)) return;
  if (!inputNumber(r, "r (m)", g_telex_mode)) return;
  if (r <= 0) { showResult("Escape", "r>0"); return; }
  double v = sqrt(2.0 * kG * M / r);
  showResult("Escape", "v = " + formatFloat(v, 6) + " m/s");
}
...
void actionGravity() {
  ...
  if (r <= 0) { showResult("Gravity", "r>0"); return; }
  double F = kG * m1 * m2 / (r * r);
  showResult("Gravity", "F = " + formatFloat(F, 6) + " N");
}

MenuItem kMenu[] = {
    {"Orbital velocity", actionOrbital},
    {"Escape velocity", actionEscape},
    {"Gravitation", actionGravity},
};
...
void drawMenu() {
  ...
  M5.Display.print("idk-physics-astro");
