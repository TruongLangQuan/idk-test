#include <M5Unified.h>
#include <cmath>

#include <idk_ui.h>
#include "generated/vi12_font.h"

namespace {

constexpr int kMenuLines = 6;

struct MenuItem {
  const char* title;
  void (*action)();
};

bool g_telex_mode = false;
String g_status = "Ready";

MemoryFontWrapper g_font_wrap(vi12_font, vi12_font_len);

constexpr KnownWifi kKnownWifis[] = {
    {"THCS NTD 02", "thcsnguyenthidinh"},
    {"THCS Nguyen Thi Dinh", "thcsnguyenthidinh"},
    {"Quan Le", "15032011"},
    {"NTD-THCS", "GIAOVIEN2425@"},
};

int g_menu_idx = 0;
int g_menu_scroll = 0;

void actionAlkane() {
  double n = 0;
  if (!inputNumber(n, "n (C atoms)", g_telex_mode)) return;
  int c = static_cast<int>(round(n));
  if (c <= 0) {
    showResult("Alkane", "n must be >= 1");
    return;
  }
  int h = 2 * c + 2;
  showResult("Alkane", "Formula: C" + String(c) + "H" + String(h));
}

void actionAlkene() {
  double n = 0;
  if (!inputNumber(n, "n (C atoms)", g_telex_mode)) return;
  int c = static_cast<int>(round(n));
  if (c <= 1) {
    showResult("Alkene", "n must be >= 2");
    return;
  }
  int h = 2 * c;
  showResult("Alkene", "Formula: C" + String(c) + "H" + String(h));
}

void actionAlkyne() {
  double n = 0;
  if (!inputNumber(n, "n (C atoms)", g_telex_mode)) return;
  int c = static_cast<int>(round(n));
  if (c <= 1) {
    showResult("Alkyne", "n must be >= 2");
    return;
  }
  int h = 2 * c - 2;
  showResult("Alkyne", "Formula: C" + String(c) + "H" + String(h));
}

void actionDBE() {
  double c = 0, h = 0, n = 0, x = 0;
  if (!inputNumber(c, "C", g_telex_mode)) return;
  if (!inputNumber(h, "H", g_telex_mode)) return;
  if (!inputNumber(n, "N", g_telex_mode)) return;
  if (!inputNumber(x, "Halogen X", g_telex_mode)) return;
  double dbe = (2 * c + 2 + n - h - x) / 2.0;
  showResult("DBE", "DBE = " + formatFloat(dbe, 2));
}

void actionGroups() {
  String text =
      "Functional groups:\n"
      "- Alkane: C-C\n"
      "- Alkene: C=C\n"
      "- Alkyne: C#C\n"
      "- Alcohol: -OH\n"
      "- Aldehyde: -CHO\n"
      "- Ketone: C=O\n"
      "- Carboxyl: -COOH\n"
      "- Ester: -COO-\n"
      "- Amine: -NH2";
  showResult("Groups", text);
}

MenuItem kMenu[] = {
    {"Alkane formula", actionAlkane},
    {"Alkene formula", actionAlkene},
    {"Alkyne formula", actionAlkyne},
    {"DBE (unsaturation)", actionDBE},
    {"Functional groups", actionGroups},
};
constexpr int kMenuCount = sizeof(kMenu) / sizeof(kMenu[0]);

void ensureScroll() {
  if (g_menu_idx < g_menu_scroll) g_menu_scroll = g_menu_idx;
  if (g_menu_idx >= g_menu_scroll + kMenuLines) g_menu_scroll = g_menu_idx - kMenuLines + 1;
}

void drawMenu() {
  ensureScroll();
  M5.Display.fillScreen(TFT_BLACK);
  M5.Display.setTextColor(TFT_WHITE, TFT_BLACK);
  M5.Display.setCursor(2, 2);
  M5.Display.print("idk-chemistry-organic");

  M5.Display.setTextColor(TFT_DARKGREY, TFT_BLACK);
  M5.Display.setCursor(2, 14);
  M5.Display.print(g_status);

  int y = 28;
  for (int i = 0; i < kMenuLines; ++i) {
    int idx = g_menu_scroll + i;
    if (idx >= kMenuCount) break;
    bool sel = (idx == g_menu_idx);
    M5.Display.setTextColor(sel ? TFT_YELLOW : TFT_WHITE, TFT_BLACK);
    M5.Display.setCursor(4, y);
    M5.Display.print(sel ? "> " : "  ");
    M5.Display.print(kMenu[idx].title);
    y += 16;
  }

  M5.Display.setTextColor(TFT_DARKGREY, TFT_BLACK);
  M5.Display.setCursor(2, 124);
  M5.Display.print("A:Select  B:Next  PWR:Prev");
}

}  // namespace

void setup() {
  auto cfg = M5.config();
  M5.begin(cfg);
  M5.Display.setRotation(3);
  M5.Display.setBrightness(180);

  (void)loadFontFromMemory(vi12_font, vi12_font_len, g_font_wrap);
  autoConnectKnownWifi(kKnownWifis, sizeof(kKnownWifis) / sizeof(kKnownWifis[0]), g_status);
  drawMenu();
}

void loop() {
  M5.update();
  if (M5.BtnB.wasPressed()) {
    g_menu_idx = (g_menu_idx + 1) % kMenuCount;
    drawMenu();
  }
  if (M5.BtnPWR.wasPressed()) {
    g_menu_idx = (g_menu_idx - 1 + kMenuCount) % kMenuCount;
    drawMenu();
  }
  if (M5.BtnA.wasPressed()) {
    kMenu[g_menu_idx].action();
    drawMenu();
  }
  delay(10);
}
