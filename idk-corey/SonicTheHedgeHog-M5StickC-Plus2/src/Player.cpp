...
void Player::movement() {
    ...
    if (ground) {
        if (action == ACT_NORMAL || action == ACT_SKID) {
            // Check for down press while moving to trigger roll
            if (Keyboard::isKeyPressedDown() && fabs(gsp) >= 0.5) {
                action = ACT_ROLL;
                pos.y += 5;
            }
            ...
            // Stand back up when down is released
            if (!Keyboard::isKeyPressedDown()) {
                action = ACT_NORMAL;
                pos.y -= 5;
            }
